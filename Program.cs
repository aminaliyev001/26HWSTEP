...
var domain_list = debtors.Where(debt => debt.Email.Contains("rhyta.com") || debt.Email.Contains("dayrep.com")).ToList();
foreach(var az in domain_list) {
    Console.WriteLine(az.Email);
}

// 3) Yashi 26-dan 36-ya qeder olan borclulari cixartmag
var now = DateTime.Now;
var now_26 = now.AddYears(-26);
var now_36 = now.AddYears(-36);

var list_age = debtors.Where(debt => debt.BirthDay <= now_26 && debt.BirthDay >= now_36).ToList();
foreach(var az in list_age) {
    Console.WriteLine(DateTime.Now.Year - az.BirthDay.Year);
}
// 4) Borcu 5000-den cox olmayan borclularic cixartmag

var borcu5000_list = debtors.Where(d => d.Debt < 5000).ToList();
foreach(var az in borcu5000_list) {
    Console.WriteLine(az.Debt);
}
//  5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2-den cox 7 reqemi olan borclulari cixartmaq

var list_18simvol = debtors.Where(debt => debt.FullName.Length > 18).Where(debt => debt.Phone.Count(z => z == '7') >= 2).ToList();
foreach(var az in list_18simvol) {
    Console.WriteLine(az.FullName + "\n" + az.Phone);
}
//7) Qishda anadan olan borclulari cixardmaq 
Console.WriteLine("\n\n Qish: ");
var list_qish = debtors.Where(debt => debt.BirthDay.Month == 12 || debt.BirthDay.Month == 1 || debt.BirthDay.Month == 2).ToList();
foreach(var az in list_qish) {
    Console.WriteLine(az.FullName);
}
...
var ortalamaBorc = debtors.Average(d => d.Debt);
var _8 = debtors.Where(az => az.Debt > ortalamaBorc).OrderBy(d => d.FullName.Split(' ')[1]).ThenByDescending(d => d.Debt).ToList();
foreach(var ab in _8) {
...
var _9 = debtors.Where(z => !z.Phone.Contains('8')).ToList();
foreach(var po in _9) {
    Console.WriteLine(po.FullName.Split(" ")[2] + " " + po.Debt);
}
...
var _13 = debtors.GroupBy(d => d.BirthDay.Year).OrderByDescending(g => g.Count()).Select(g => g.Key).FirstOrDefault();
...
foreach(var az in debtors) {
    Console.WriteLine(az.FullName);
}

// 15)Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq
var allborc = debtors.Sum(d => d.Debt);
Console.WriteLine("Umumi borc: " + allborc);

// 16)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
var dunyamuharibesi = debtors.Where(d => d.BirthDay.Year <= 1940).ToList();
foreach(var az in dunyamuharibesi) {
    Console.WriteLine(az.FullName);
}

// 18)Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq
var _tapsiriq18 = debtors.Where(d => d.Phone.Distinct().Count() == d.Phone.Length).ToList();
foreach(var az in _tapsiriq18) {
    Console.WriteLine(az.FullName + " " + az.Debt);
}
...
var task19 = debtors.Where(d => d.Debt <= (500 * 6)).ToList();